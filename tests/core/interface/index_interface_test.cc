...
// Verify that group_by search works end-to-end through the core Index
...
TEST(IndexInterface, GroupBySearch) {
  ...
  auto func = [&](const BaseIndexParam::Pointer &param,
    ...
    // With query [1,1,1,1] (IP), score(i) = 4*i.
    for (uint32_t i = 0; i < kNumDocs; ++i) {
    ...
    // Set up group_by: group(i) = to_string(i % kNumGroups)
    ...
    query_param->group_by_param->group_by = [&](uint64_t key) -> std::string {
      return std::to_string(key % kNumGroups);
    };

    // Search
    std::vector<float> query_vec(kDimension, 1.0f);
    VectorData query{DenseVector{query_vec.data()}};
    SearchResult result;
    ASSERT_EQ(0, index->Search(query, query_param, &result));

    // When group_by is active, group_doc_list_ should be populated
    // and doc_list_ should be empty.
    ...
    for (const auto &group : result.group_doc_list_) {
      ...
      // Each group should have at most group_topk documents
      ASSERT_LE(group.docs().size(), kGroupTopk);
      // Each group should have at least 1 document
    ...
    // Verify documents within each group are sorted by score (descending for
    // IP) and belong to the correct group.
    for (const auto &group : result.group_doc_list_) {
      ...
      // After normalization, IP scores are non-increasing within a group
      // (best result first).
      for (size_t j = 1; j < group.docs().size(); ++j) {
        ASSERT_GE(group.docs()[j - 1].score(), group.docs()[j].score())
            << "Docs not sorted by score within group " << group.group_id();
      }
    }

    // Verify fetch_vector with group_by: when fetch_vector is enabled,
    // each doc in each group should have a valid vector pointer, and
    // the fetched vectors should match the original inserted data.
    if (query_param->fetch_vector) {
      ...
      for (const auto &group : result.group_doc_list_) {
        for (const auto &doc : group.docs()) {
          ...
          if (!result.reverted_vector_list_.empty()) {
            // Reformer path (e.g. quantized): reverted_vector_list_ is
            // flattened across all groups in iteration order.
            ASSERT_LT(reverted_idx, result.reverted_vector_list_.size());
            auto reverted = reinterpret_cast<const float *>(
                result.reverted_vector_list_[reverted_idx].data());
            for (uint32_t d = 0; d < kDimension; ++d) {
              ASSERT_FLOAT_EQ(expected_val, reverted[d])
                  << "Reverted vector mismatch for doc " << doc.key() << " dim "
                  << d;
            }
            ++reverted_idx;
          } else {
            // Direct path (FP32): vector accessible via doc.vector()
            ASSERT_NE(nullptr, doc.vector())
                << "Vector should be fetched for doc " << doc.key();
            auto vec = reinterpret_cast<const float *>(doc.vector());
            for (uint32_t d = 0; d < kDimension; ++d) {
              ASSERT_FLOAT_EQ(expected_val, vec[d])
                  << "Fetched vector mismatch for doc " << doc.key() << " dim "
                  << d;
            }
          }
        }
      }
      // If revert path was used, total count should match
      if (!result.reverted_vector_list_.empty()) {
        size_t total_docs = 0;
        for (const auto &group : result.group_doc_list_) {
          total_docs += group.docs().size();
        }
        ASSERT_EQ(total_docs, result.reverted_vector_list_.size());
      }
    }
  ...
  func(FlatIndexParamBuilder()
           ...
           .WithIsSparse(false)
           .Build(),
       FlatQueryParamBuilder().with_topk(100).build());

  // Flat index with fetch_vector
  func(FlatIndexParamBuilder()
           .WithMetricType(MetricType::kInnerProduct)
           .WithDataType(DataType::DT_FP32)
           .WithDimension(kDimension)
           .WithIsSparse(false)
           .Build(),
       FlatQueryParamBuilder().with_topk(100).with_fetch_vector(true).build());
  ...
  func(HNSWIndexParamBuilder()
           ...
           .WithEFConstruction(100)
           .Build(),
       HNSWQueryParamBuilder().with_topk(100).with_ef_search(100).build());

  // HNSW index with fetch_vector
  func(HNSWIndexParamBuilder()
           .WithMetricType(MetricType::kInnerProduct)
           .WithDataType(DataType::DT_FP32)
           .WithDimension(kDimension)
  ...
  auto sparse_func = [&](const BaseIndexParam::Pointer &param,
    ...
    // Each doc i gets sparse vector with kDimension non-zero entries,
    // all set to float(i).  IP with query [1,1,...,1] = i * kDimension.
    for (uint32_t i = 0; i < kNumDocs; ++i) {
      std::vector<uint32_t> indices(kDimension);
      std::vector<float> values(kDimension, static_cast<float>(i));
      std::iota(indices.begin(), indices.end(), 0u);
      VectorData data{SparseVector{kDimension, indices.data(), values.data()}};
      ASSERT_EQ(0, index->Add(data, i));
    }
    ASSERT_EQ(0, index->Train());
    ...
    std::vector<uint32_t> q_indices(kDimension);
    std::vector<float> q_values(kDimension, 1.0f);
    std::iota(q_indices.begin(), q_indices.end(), 0u);
    VectorData query{
        SparseVector{kDimension, q_indices.data(), q_values.data()}};
    SearchResult result;
    ASSERT_EQ(0, index->Search(query, query_param, &result));
  ...
  // Flat sparse index
  sparse_func(FlatIndexParamBuilder()
                  .WithMetricType(MetricType::kInnerProduct)
                  .WithDataType(DataType::DT_FP32)
                  .WithIsSparse(true)
                  .Build(),
  ...
  // Flat sparse index with fetch_vector
  sparse_func(
      FlatIndexParamBuilder()
          .WithMetricType(MetricType::kInnerProduct)
          .WithDataType(DataType::DT_FP32)
  ...
  // HNSW sparse index with fetch_vector
  sparse_func(HNSWIndexParamBuilder()
                  .WithMetricType(MetricType::kInnerProduct)
                  .WithDataType(DataType::DT_FP32)
                  .WithIsSparse(true)
...
#if RABITQ_SUPPORTED
// Verify that group_by search works with the HNSWRabitq index type.
// This uses the same 12-doc grouping scheme as the dense/sparse tests above
// but requires the Rabitq-specific provider and reformer setup.
TEST(IndexInterface, GroupBySearchHNSWRabitq) {
  using namespace zvec::core;
  using namespace zvec::ailego;
  ...
  constexpr uint32_t kNumGroups = 3;
  constexpr uint32_t kGroupTopk = 2;
  const std::string index_name{"test_groupby_rabitq.index"};
  const std::string cleanup_pattern = index_name + "*";

  // Build provider with training vectors (500 docs for RaBitQ training).
  // Docs 0..kNumDocs-1 will be inserted into the index; the rest are
  // only used for converter training.
  constexpr size_t kTrainCount = 500;
  auto holder = std::make_shared<
      zvec::core::MultiPassIndexProvider<IndexMeta::DataType::DT_FP32>>(
      kDimension);
  for (size_t i = 0; i < kTrainCount; ++i) {
    NumericalVector<float> vec(kDimension, static_cast<float>(i));
    ASSERT_TRUE(holder->emplace(i, vec));
  }

  // Train RabitqConverter on the same data.
  auto index_meta =
      std::make_shared<IndexMeta>(IndexMeta::DataType::DT_FP32, kDimension);
  index_meta->set_metric("InnerProduct", 0, Params());
  RabitqConverter converter;
  converter.init(*index_meta, Params());
  ASSERT_EQ(converter.train(holder), 0);
  std::shared_ptr<IndexReformer> reformer;
  ASSERT_EQ(converter.to_reformer(&reformer), 0);

  zvec::test_util::RemoveTestFiles(cleanup_pattern);
  auto index = zvec::core_interface::IndexFactory::CreateAndInitIndex(
      *HNSWRabitqIndexParamBuilder()
           .WithMetricType(MetricType::kInnerProduct)
           .WithDataType(DataType::DT_FP32)
           .WithDimension(kDimension)
           .WithIsSparse(false)
  ...
  // Set up group_by: group(i) = to_string(i % kNumGroups)
  auto query_param =
      HNSWRabitqQueryParamBuilder().with_topk(100).with_ef_search(100).build();
  query_param->group_by_param = std::make_shared<GroupByParam>();
  query_param->group_by_param->group_count = kNumGroups;
  query_param->group_by_param->group_topk = kGroupTopk;
  query_param->group_by_param->group_by = [&](uint64_t key) -> std::string {
...
// Verify that when an index type that does NOT support group_by is used with
// a group_by_param, the search does not populate group_doc_list_.
// This guards against silently returning no results when the algorithm
// ignores the group_by context.
TEST(IndexInterface, GroupByUnsupportedIndexType) {
  constexpr uint32_t kDimension = 4;
  constexpr uint32_t kNumDocs = 12;
